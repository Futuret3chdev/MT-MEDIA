import { PokerGame } from './game.js?v=53';
import { PokerUI } from './ui.js?v=53';
import { TABLE_MODES, FLOOR_MODES, CASINO_GAME_SECTIONS, MULTIPLAYER_ROOMS } from './modes.js?v=53';
import { floorSymbol } from './floor-format.js?v=53';
import { artForGame } from './game-art.js?v=53';
import { applyGameScene } from './game-scene.js?v=53';
import { RouletteUI } from './roulette-ui.js?v=53';
import { BlackjackUI } from './blackjack-ui.js?v=53';
import { StarfallUI } from './starfall-ui.js?v=53';
import { BaccaratUI } from './baccarat-ui.js?v=53';
import { casinoSound, unlockAudio } from './sounds.js?v=53';
import { celebrateWin, winTier } from './celebration.js?v=53';
import { isAgeVerified, openAgeGate, bindAgeGate } from './gate.js?v=53';
import {
...
import { generateRoomCode, simulateMatchmaking } from './multiplayer.js?v=53';
import { detectWallets, sendMTToTreasury } from './solana-wallet.js?v=53';
import { MEMETORRENT, LUCKY_REELS_URL } from './config.js?v=53';
import {
  loadProfile, updateProfile, uploadAvatarFile, removeAvatar,
  CHARACTER_PRESETS, getDisplayName, isSignedIn
} from './profile.js?v=53';
import { renderAvatarHTML } from './avatar.js?v=53';
import {
...
let floorSession = null;
let floorBalanceAdapter = null;
...
function resolveLobbyMode(game, sectionId) {
  if (!game.modeId) return null;
  if (sectionId === 'poker') return TABLE_MODES[game.modeId] || null;
  if (sectionId === 'floor') return FLOOR_MODES[game.modeId] || null;
  return null;
}

function floorStakesText(mode) {
  const sym = floorSymbol(mode);
  if (mode.currency === 'mt') {
    return `${mode.buyIn} ${sym} buy-in · max ${mode.maxBet} ${sym}`;
  }
  return `Free chips · max ${mode.maxBet} ${sym}`;
}

function createFloorBalance(mode) {
  if (mode.currency === 'free') {
    return {
  ...
  let bank = floorSession?.bank ?? mode.buyIn;
  return {
    getBalance: () => bank,
    onBalanceChange: (delta) => {
      bank = Math.max(0, bank + delta);
      floorSession = { bank, mode };
    },
    destroy: async () => {
...
function updateFloorBankLabel(prefix, mode) {
  const symEl = document.getElementById(`${prefix}-currency`);
  if (symEl) symEl.textContent = mode.currency === 'mt' ? 'MT ' : '₵';
  const badge = document.getElementById(`${prefix}-mode-badge`);
  if (badge) {
    badge.textContent = mode.badge || (mode.currency === 'mt' ? '$MT' : 'FREE');
    badge.dataset.currency = mode.currency;
  }
}

async function endFloorSession() {
  if (floorBalanceAdapter?.destroy) {
    await floorBalanceAdapter.destroy();
  }
  floorBalanceAdapter = null;
  currentMode = null;
}

function selectCasinoMode(mode) {
  ...
  document.getElementById('setup-desc').textContent = mode.subtitle;
  document.getElementById('setup-buyin').textContent = `${mode.buyIn} ${sym}`;
  document.getElementById('setup-blinds').textContent = `Max bet ${mode.maxBet} ${sym}`;
  document.getElementById('setup-stack').textContent = `${mode.buyIn} ${sym}`;

  const mtWarn = document.getElementById('setup-mt-warn');
  if (mtWarn) {
    mtWarn.style.display = '';
    mtWarn.textContent = `Buy-in sends ${mode.buyIn} MT to treasury — session bank for this table`;
  }
...
async function launchFloorGame() {
  ...
  try {
    if (mode.currency === 'mt') {
      ...
      if (mode.buyIn > 0) {
        ...
        floorSession = { bank: mode.buyIn, mode };
      }
    }
    ...
    if (launchBtn) {
      ...
      launchBtn.textContent = 'Take a Seat';
    }
  }
}

function openFloorGame(mode) {
  currentMode = mode;
  if (mode.game === 'blackjack') openBlackjack(mode);
  else if (mode.game === 'roulette') openRoulette(mode);
  else if (mode.game === 'baccarat') openBaccarat(mode);
}

function renderGameCard(game, sectionId) {
  const mode = resolveLobbyMode(game, sectionId);
  ...
  const playAction = () => {
    if (!requireCasinoAccess()) return;
    if (mode?.game) selectCasinoMode(mode);
    else if (mode) selectMode(mode);
...
function openBlackjack(mode = FLOOR_MODES['blackjack-free']) {
  if (!requireCasinoAccess()) return;
  applyGameScene('blackjack-scene', mode.id);
  unlockAudio();
  stopBlackjack(false);
  floorBalanceAdapter = createFloorBalance(mode);
  updateFloorBankLabel('bj', mode);
  blackjackUI = new BlackjackUI({ mode, ...floorBalanceAdapter });
  showScreen('blackjack');
}

async function stopBlackjack(endSession = true) {
  blackjackUI?.destroy();
  blackjackUI = null;
  if (endSession) await endFloorSession();
}
...
function openBaccarat(mode = FLOOR_MODES['baccarat-free']) {
  if (!requireCasinoAccess()) return;
  applyGameScene('baccarat-scene', mode.id);
  unlockAudio();
  stopBaccarat(false);
  floorBalanceAdapter = createFloorBalance(mode);
  updateFloorBankLabel('bc', mode);
  baccaratUI = new BaccaratUI({ mode, ...floorBalanceAdapter });
  showScreen('baccarat');
}

async function stopBaccarat(endSession = true) {
  baccaratUI?.destroy();
  baccaratUI = null;
  if (endSession) await endFloorSession();
}

function openRoulette(mode = FLOOR_MODES['roulette-free']) {
  if (!requireCasinoAccess()) return;
  applyGameScene('roulette-scene', mode.id);
  unlockAudio();
  stopRoulette(false);
  floorBalanceAdapter = createFloorBalance(mode);
  updateFloorBankLabel('rl', mode);
  rouletteUI = new RouletteUI({ mode, ...floorBalanceAdapter });
  showScreen('roulette');
  requestAnimationFrame(() => rouletteUI?.wheelRenderer?.resize());
}

async function stopRoulette(endSession = true) {
  rouletteUI?.destroy();
  rouletteUI = null;
  if (endSession) await endFloorSession();
}
...
document.getElementById('btn-blackjack-back')?.addEventListener('click', async () => {
  await stopBlackjack();
  showScreen('menu');
});
...
document.getElementById('btn-baccarat-back')?.addEventListener('click', async () => {
  await stopBaccarat();
  showScreen('menu');
});
...
document.getElementById('btn-roulette-back')?.addEventListener('click', async () => {
  await stopRoulette();
  showScreen('menu');
});
