...
import { spawnAmbient, burstAt, showBanner } from './game-fx.js?v=53';
...
export class BlackjackUI {
  constructor({ getBalance, onBalanceChange, mode = FLOOR_MODES['blackjack-free'] }) {
    this.mode = mode;
  ...
  buildChips() {
    ...
    this.els.chips.innerHTML = this.chips.map((v) => `
      <button type="button" class="bj-chip${v === this.chip ? ' on' : ''}" data-chip="${v}">
        <span>${formatFloorChipLabel(this.mode, v)}</span>
      </button>
    `).join('');
  ...
  renderChipStack() {
    ...
    this.els.chipStack.innerHTML = this.chipStack.map((v, i) => `
      <span class="bj-stack-chip" data-v="${v}" style="--i:${i}">${formatFloorChipLabel(this.mode, v)}</span>
    `).join('');
  ...
  setPhase(phase) {
    ...
    this.els.deal.disabled = !betting || this.bet < this.minBet;
  ...
  addBet() {
    ...
    if (this.bet + this.chip > this.maxBet) {
      this.setMessage(`Table max bet ${formatFloorAmt(this.mode, this.maxBet)}`, 'lose');
      return;
    }
    this.bet += this.chip;
  ...
  spawnChipAnim() {
    ...
    chip.textContent = formatFloorChipLabel(this.mode, this.chip);
  ...
  async dealHand() {
    if (this.bet < this.minBet || this.phase !== 'bet') return;
    this.player = [];
    this.dealer = [];
    this.doubled = false;
    this.dealerHidden = true;
  ...
  finishRound() {
    ...
    if (result.payout > 0) {
      ...
      if (tier !== 'small') celebrateWin(result.payout, tier, this.sym);
