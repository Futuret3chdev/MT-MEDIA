...
import { casinoSound } from './sounds.js?v=51';
...
export class RouletteUI {
  constructor({ onBalanceChange, getBalance, mode = FLOOR_MODES['roulette-free'] }) {
    this.mode = mode;
    this.chipValues = mode.chipValues;
    this.sym = floorSymbol(mode);
    this.onBalanceChange = onBalanceChange;
    this.getBalance = getBalance;
    this.chipValue = this.chipValues[1] ?? this.chipValues[0];
    this.bets = [];
    this.spinning = false;
    this.wheelDeg = 0;
    this.history = [];
  ...
  buildChips() {
    this.els.chipSelect.innerHTML = this.chipValues.map((v) => `
      <button type="button" class="rl-chip-pick${v === this.chipValue ? ' on' : ''}" data-chip="${v}">
        <span class="rl-chip-disc"></span>
        <span class="rl-chip-amt">${formatFloorChipLabel(this.mode, v)}</span>
      </button>
    `).join('');
  }
  ...
  updateSummary() {
    ...
    this.els.summary.textContent = `Bets: ${formatFloorAmt(this.mode, total)}`;
  }
  ...
  async spin() {
    ...
    if (totalReturn > staked) {
      ...
      this.setMessage(`Winner! +${formatFloorAmt(this.mode, profit)}`);
    } else if (totalReturn > 0) {
      casinoSound.call();
      this.setMessage(`${formatFloorAmt(this.mode, totalReturn)} returned`);
    } else {
      casinoSound.lose();
